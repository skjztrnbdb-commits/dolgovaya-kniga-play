...
if(typeof window!=='undefined'){
  window.__DK_TEST__={
    ...
    getL:()=>L,getLANG:()=>LANG,
    PH_TABS,PRESETS,TIERS,POOL,ACHS,GOLD_PERM,REWARD,UPSTEP,ORDER,BUILD,exportLog,TAR_COST,DUTY_FAIL,maxBuyQty,MAX_FILL,TOAST_MAX
  };
}

/* ============ офлайн ============
   Регистрируем сервис-воркер (game/sw.js) — он кладёт игру в кэш телефона, и она
   открывается без сети. Три оговорки в условии: в тестовой песочнице navigator
   вообще нет; по file:// воркеры запрещены браузером (а игра обязана работать
   и просто открытым файлом); ошибку регистрации глотаем — офлайн приятен,
   но игра без него ничего не теряет. */
