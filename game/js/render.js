...
function balanceChartSVG(){
  ...
  // Подпись идёт ПОД линией и на узком экране сокращается до одного слова: полная
  // («резерв на новое дело · $6 000») лезла на плашку ближайшей ревизии и читалась
  // как каша из двух подписей (Сергей, 28.08). Что это за линия, объяснено
  // в легенде под графиком — здесь нужно только назвать её.
  s+='<line x1="'+PL+'" y1="'+yr+'" x2="'+(W-PR)+'" y2="'+yr+'" stroke="#2C6B4F" stroke-width="1.4" stroke-dasharray="4 4" opacity=".9"/>'+
     '<text x="'+(W-PR)+'" y="'+(yr+11)+'" text-anchor="end" fill="#2C6B4F">'+
     (NARROW()?L.chart.axisReserveShort:L.chart.axisReserve(rt))+'</text>';
...
function draw(){
  ...
  // На телефоне сноска короткая: перечисление «Соль — 1 место за единицу; …»
  // дословно повторяет строку в каждой карточке, и внизу листа читается как
  // случайный кусок чужого текста (Сергей, 30.08). Остаётся то, чего в карточках
  // нет, — потолок одной покупки.
  el('tbl-note').textContent=NARROW()
    ? L.tbl.noteShort(lotCap())
    : L.tbl.note(GOODS.slice(0,3).map(g=>L.tbl.noteItem(g.n,g.bulk)).join('; '),lotCap());
  ...
  GOODS.forEach(g=>{
    ...
    // lot-btn/all-btn — не оформление, а метки: на телефоне «Партию» переставляют
    // в строку счётчика, а «Всё» прячут вместе с «Продать», когда продавать нечего
    // (см. render-phone.js). Классы нужны, чтобы находить кнопки без счёта по порядку
    const b2=document.createElement('button');b2.className='buy lot-btn';
    const b3=document.createElement('button');b3.className='sell';
    const b4=document.createElement('button');b4.className='sell all-btn';
  ...
  // раскладка телефона: перед обучением, потому что подсветка шага ищет элементы
  // на листах и должна найти их уже разложенными (см. render-phone.js)
  if(typeof phoneShell==='function')phoneShell();
...
/* ============ настройки (внутри игры) ============
   Содержимое собирается отдельно от оболочки: на десктопе оно лежит во всплывающем
   слое, на телефоне — на пятой вкладке (render-phone.js). Один и тот же код на оба
   случая, иначе настройки на телефоне тихо отставали бы от настроек на компьютере.
   `onClose` — что делать по кнопке «Закрыть»: закрыть слой или вернуться на лист. */
function settingsBody(box,rerender,onClose,extra){
  const h=document.createElement('h2');h.textContent=L.set.title;h.style.cssText='text-align:center';
  box.appendChild(h);
  // Кнопка закрытия сверху И снизу: список достижений длинный, и промотав его
  // вниз, возвращаться наверх ради выхода — лишняя работа; а закрыть, ничего
  // не листая, наоборот, хочется сразу (Сергей, 29.08)
  const mkClose=()=>{
    const b=document.createElement('button');b.className='wide dk-set-close';
    b.textContent=L.set.close;b.onclick=onClose;return b;
  };
  box.appendChild(mkClose());
  const sndRow=document.createElement('button');sndRow.className='wide';
  sndRow.textContent=L.set.sound(sndOn);
  sndRow.onclick=()=>{sndOn=!sndOn;if(sndOn)snd('click');rerender()};
  ...
  // сюда телефон досыпает то, что на компьютере живёт в подвале: журнал и меню
  if(extra)extra.forEach(n=>box.appendChild(n));
  box.appendChild(buildAchGrid(true,rerender));
  const bottom=mkClose();bottom.style.marginTop='10px';
...
function settings(){
  ...
  settingsBody(box,rerenderSettings,()=>{o.remove();unstretchRoot()});
  o.appendChild(box);
  el('root').appendChild(o);bringIntoView();stretchRootFor(o);
}
