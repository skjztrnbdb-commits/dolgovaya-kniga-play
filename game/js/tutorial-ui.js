...
function tutFocus(){
  ...
  if(list.length&&!TUT.scrolled){
    ...
    const target=list[0];
    // на телефоне цель может лежать на другом листе — открываем его, иначе шаг
    // подсвечивает и прокручивает то, чего сейчас нет на экране
    if(typeof phoneReveal==='function')phoneReveal(target);
